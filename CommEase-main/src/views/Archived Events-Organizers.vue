          ...
          <div class="archived-contents-separation">
            ...
            <div class="archived-contents">
              ...
              <p class="archived-event-participants">
                ...
                {{ event.registered_count || 0 }}/{{ event.participant_limit || 0 }}
              </p>
              <span class="archived-event-programs">
                <span><b>Programs:</b></span> {{ (event.programs || []).join(', ') }}
              </span>
            </div>
          </div>
...
export default {
  setup() {
      ...
      try {
        ...
        try {
          ...
          if (user.role !== 'organizer') {
            error.value = `Access denied. Current role: "${user.role}" (type: ${typeof user.role}). Expected: "organizer"`;
            return;
          }
          ...
          console.log('Regular events route works:', testResponse.data.length, 'events');
    ...
    const deleteEvent = async (eventId, index) => {
      const confirmed = confirm("Do you want to permanently delete this archived event?");
      if (confirmed) {
        try {
          loading.value = true;
